...
import useSocket from "@/app/hooks/useSocket"; // Pastikan path ini sesuai
...
export default function ChatLayout({ user }) {
  ...
  const handleSendMessage = () => {
    ...
    // Jika ada gambar, kita perlu mengirimnya secara terpisah
    // Untuk sementara, kita hanya mengirim teks
    sendMessage(newMsg);
    setNewMsg("");
  ...
  const saveEdit = (id) => {
    ...
    socket.emit("editMessage", { id, text: editText.trim() });
    setEditingId(null);
    setEditText("");
  };
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="flex justify-between items-center bg-blue-600 text-white p-4 shadow-md">
        <div className="flex items-center space-x-3">
          ...
          <div className={`h-3 w-3 rounded-full ${connectionStatusColors[connectionStatus]}`}></div>
          <span className="text-sm hidden sm:inline">{connectionStatusText[connectionStatus]}</span>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <span className="hidden md:inline">Hai, {user.username || user.displayName}</span>
          ...
          <div className="flex flex-wrap gap-2">
            {onlineUsers.map((username, index) => (
              <div key={index} className="flex items-center bg-blue-100 px-3 py-1 rounded-full">
                ...
                <span className={`text-sm ${username === (user.username || user.displayName) ? "font-bold text-blue-600" : "text-gray-700"}`}>
                  ...
                  {editingId === (msg._id || msg.id) ? (
                    <div className="flex flex-col space-y-2">
                      <input
                        ...
                        autoFocus
                        onKeyDown={(e) => {
                          if (e.key === "Enter") saveEdit(msg._id || msg.id);
                          if (e.key === "Escape") setEditingId(null);
                        }}
                      />
                      <div className="flex space-x-2 self-end">
                        <button onClick={() => saveEdit(msg._id || msg.id)} className="bg-green-500 px-3 py-1 rounded text-white text-sm">
                          Simpan
                        </button>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-xs font-bold opacity-80">{msg.senderName || msg.username}</span>
      ...
      <div className="flex flex-col p-4 space-y-2 border-t border-gray-300 bg-white">
        <div className="flex space-x-2">
          ...
          <button 
            ...
            disabled={(!newMsg.trim() && !selectedImage) || isSending}
