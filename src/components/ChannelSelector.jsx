"use client";

import { useState } from "react";

export default function ChannelSelector({ 
  user, 
  channels, 
  loading, 
  selectedChannelId, 
  onSelectChannel, 
  onRefetch, 
  onCreateChannel, 
  onLogout, 
  error 
}) {
  const [showMenu, setShowMenu] = useState(false);

  return (
    <div className="h-full flex flex-col">
      {/* Header dengan tombol buat channel dan logout */}
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white">
        <h2 className="text-lg font-semibold">Channels</h2>
        <div className="flex items-center space-x-2">
          {/* Tombol Buat Channel - PASTIKAN INI ADA DAN BERFUNGSI */}
          <button
            onClick={onCreateChannel}
            className="p-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors"
            title="Buat Channel Baru"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
          </button>
          
          {/* Tombol Menu (untuk logout dan refresh) */}
          <div className="relative">
            <button
              onClick={() => setShowMenu(!showMenu)}
              className="p-2 bg-gray-200 rounded-full hover:bg-gray-300 transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            
            {showMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10">
                <button
                  onClick={onRefetch}
                  className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left"
                >
                  Refresh Channels
                </button>
                <button
                  onClick={onLogout}
                  className="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 w-full text-left"
                >
                  Logout
                </button>
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Daftar Channels */}
      <div className="flex-1 overflow-y-auto p-2">
        {loading ? (
          <div className="flex justify-center items-center h-20">
            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div>
          </div>
        ) : error ? (
          <div className="p-4 text-red-500 bg-red-50 rounded-md text-sm">
            {error}
          </div>
        ) : channels.length === 0 ? (
          <div className="p-4 text-center text-gray-500">
            Tidak ada channels. Klik tombol + untuk membuat channel baru.
          </div>
        ) : (
          <div className="space-y-1">
            {channels.map((channel) => (
              <button
                key={channel._id || channel.id}
                onClick={() => onSelectChannel(channel._id || channel.id)}
                className={`w-full text-left p-3 rounded-md transition-colors ${
                  (channel._id === selectedChannelId || channel.id === selectedChannelId)
                    ? 'bg-blue-500 text-white'
                    : 'hover:bg-gray-200'
                }`}
              >
                <div className="font-medium">#{channel.name}</div>
                {channel.description && (
                  <div className="text-xs opacity-75 truncate">{channel.description}</div>
                )}
              </button>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
