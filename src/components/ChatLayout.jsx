...
export default function ChatLayout({ user, channelId, logout }) {
  ...
  const sendMessage = useCallback(() => {
    ...
    const onMessageSent = (response) => {
      if (!response || response?.error) {
        ...
        toast.error(response?.error || "Failed to send message: Connection issue.");
      } else {
        setNewMsg("");
        setSelectedImage(null);
        setImagePreview(null);
    ...
    if (selectedImage) {
      ...
      reader.onload = async (e) => {
        ...
        console.log(`Sending image with size: ${messageData.image.length} bytes`);
  ...
  // Early return for invalid channel
  if (!channelId) {
    return (
